...
inline std::string StyledDiffHeader(std::string_view text) {
    return utils::Colorizer::Enabled().Apply(
        text, utils::TerminalStyle::Foreground(utils::TerminalColor::Cyan).Bold());
}

inline std::string StyledDiffHunk(std::string_view text) {
    return utils::Colorizer::Enabled().Apply(text,
                                             utils::TerminalStyle::Foreground(utils::TerminalColor::Yellow));
}
