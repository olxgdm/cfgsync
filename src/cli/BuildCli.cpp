...
void BuildCli(CLI::App& app, core::Registry& registry, storage::StorageManager& storageManager) {
    ...
    initCommand->add_option("--storage", *storageRoot, "Directory where cfgsync data will be stored.")
        ->required();
    initCommand->callback([&registry, &storageManager, storageRoot]() {
        commands::InitCommand command{registry, storageManager};
    ...
    addCommand->callback([&registry, addFile]() {
    ...
    removeCommand->add_option("file", *removeFile, "Tracked file path to remove from the registry.")
        ->required();
    removeCommand->callback([&registry, removeFile]() {
    ...
    listCommand->callback([&registry]() {
        commands::ListCommand command{registry};
        command.Execute();
    });

    auto* backupCommand = app.add_subcommand("backup", "Copy tracked files into storage.");
    backupCommand->callback([&registry, &storageManager]() {
        commands::BackupCommand command{registry, storageManager};
        command.Execute();
    });
    ...
    restoreCommand->callback([&registry, &storageManager, restoreAll, restoreFile]() {
        commands::RestoreCommand command{registry, storageManager};
